...
async function serverSession(){const r=await fetchJson('/api/pin-login',{method:'GET'});return r?.data?.session||null}
async function secureCall(name,params){const s=await serverSession();if(!s)return{success:false,error:'Akses operasional diperlukan untuk fitur ini.'};const endpoint=name==='getAwardee360'?'/api/awardee360':'/api/secure';return sanitizeResult(await fetchJson(endpoint,{method:'POST',body:JSON.stringify({function:name,params:params==null?null:params})}))}
