...
module.exports=async function handler(req,res){
  try{
    ...
    await store.saveConfig({clientId,clientSecret,refreshToken:current?.refreshToken||'',pendingState:state},{connectedAt:current?.refreshToken?new Date().toISOString():null,account:EXPECTED_EMAIL,lastAttemptAt:new Date().toISOString(),requestedScopes:['spreadsheets.readonly']});
    const params=new URLSearchParams({client_id:clientId,redirect_uri:REDIRECT_URI,response_type:'code',scope:SCOPE,access_type:'offline',prompt:'consent',login_hint:EXPECTED_EMAIL,state});
