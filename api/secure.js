...
function deny(res){res.status(401);res.setHeader('Content-Type','application/json; charset=utf-8');res.setHeader('Cache-Control','no-store');res.end(JSON.stringify({success:false,error:'PIN Superadmin diperlukan untuk mengubah data absensi.'}))}
module.exports=async function handler(req,res){
 const fn=String(req.body?.function||'');
 if(ATTENDANCE_PIN_REQUIRED.has(fn)&&!session.verify(req))return deny(res);
