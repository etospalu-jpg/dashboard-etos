...
module.exports=async function handler(req,res){
 ...
 try{
  ...
  const a=await resolveAwardee(req.body?.params?.id_awardee||req.body?.params||'',t);if(!a)return out(res,404,{success:false,error:'Awardee tidak ditemukan.'});
  const id=a.id;
  const [contact,academic,organizations,achievements,coaching,assessments,cases,analyses,competencies,portfolio,reflections,attendance,networking,spiritual,idp]=await Promise.all([
  ...
  const response={
   awardee:a,contact:contact?.[0]||null,academic:academic||[],akademik:academic||[],organizations:organizations||[],organisasi:organizations||[],achievements:achievements||[],prestasi:achievements||[],coaching:coaching||[],assessments:assessments||[],cases:cases||[],analyses:analyses||[],competencies:competencies||[],portfolio:portfolio?.[0]||null,reflections:reflections||[],attendance:att,networking:networking||[],spiritual:spiritual||[],idp:idp?.[0]||null,
