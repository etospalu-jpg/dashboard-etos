...
module.exports=async function handler(req,res){try{
 ...
 const user=await q('/auth/v1/user',token);if(!user?.id)return out(res,401,{success:false,error:'Session Auth tidak valid.'});
 const rows=await q(`/rest/v1/profiles?id=eq.${encodeURIComponent(user.id)}&select=id,full_name,email,role,is_active,last_login_at`,token),profile=rows?.[0];
