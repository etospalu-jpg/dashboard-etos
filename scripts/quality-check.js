...
const required=['index.html','shell.html','app.css','app-layout.css','supabase-adapter.js','app-core.js','app-public.js','app-secure.js','app-admin.js','app-360.js','app-system.js','app-data-center.js','app-access.js','app-access-control.js','app-auth-recovery.js','app-auth-rbac.js','data-center-rbac.js','api/system.js','api/data-center.js','api/access-control.js','api/auth-bridge.js','api/awardee360.js','api/idp-live.js','server-session.js','pin-login-cookie.js'];
...
try{const index=fs.readFileSync(path.join(ROOT,'index.html'),'utf8');for(const token of ['etos-build','app-data-center','app-system','app-access','app-access-control','app-auth-recovery','app-auth-rbac','etosRecoveryReady','etos-booting'])if(!index.includes(token))fail(`index.html kehilangan marker/module: ${token}`);if(index.indexOf('app-auth-recovery')>index.indexOf('app-auth-rbac'))fail('Recovery module harus dimuat sebelum RBAC utama.');if(!failures.some(x=>x.includes('index.html')||x.includes('Recovery module')))ok('Boot/module markers dan urutan recovery index.html lengkap')}catch(e){fail(`index.html tidak dapat diperiksa: ${e.message}`)}
