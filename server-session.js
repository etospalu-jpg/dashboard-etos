...
function credential(req){const s=verify(req);if(s){const k=secret(),t=kind(k);if(t==='secret'||t==='legacy')return{server:true,key:k,kind:t,session:s}}const auth=String(req.headers?.authorization||'').replace(/^Bearer\s+/i,'').trim();return auth?{server:false,token:auth}:null}
...
module.exports={PROJECT_URL,PUBLISHABLE_KEY,secret,kind,verify,setCookie,clearCookie,credential,dbHeaders,testDatabase,MAX_AGE};
